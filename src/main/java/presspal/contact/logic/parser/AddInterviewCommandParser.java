...
public class AddInterviewCommandParser implements Parser<AddInterviewCommand>{
    ...
    public AddInterviewCommand parse(String args) throws ParseException {
        ...
        // Leave it as string for now, can create object class if we really want to, depends on interview class

        //Interview interview = new Interview(date, time, location, header, index); // Omit for now since Interview class not created

        return new AddInterviewCommand(date + time + location + header + index); // Replace with interview object when Interview class is created
    }
